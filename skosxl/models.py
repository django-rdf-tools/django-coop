...
class Scheme(models.Model):
    pref_label  = models.CharField(_(u'preferred label'),blank=True,max_length=255)#devrait s'appeller juste label
    ...
    def save(self,skip_name_lookup=False, *args, **kwargs):
        ...
        if not skip_name_lookup:
            try:
                ...
                label = lookup_label.label.name
            except LabelProperty.DoesNotExist:
                label =  '< missing label >'
...
class Label(TagBase):
    ...
    def create_concept_from_label(self):
        if not self.concept:
            ...
            c.save(skip_name_lookup=True)#on vient d'indiquer pref_label
            self.concept = c
            self.save()
